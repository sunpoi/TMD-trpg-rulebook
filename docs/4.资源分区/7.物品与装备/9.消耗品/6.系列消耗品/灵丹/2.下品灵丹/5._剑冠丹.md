# 5. 剑冠丹

<table class="bannerparthead"><tbody><tr id="hdr"><td class="runninghead" nowrap="">TMD：绯红庆典041</td></tr></tbody></table>

# 5\. 剑冠丹

消耗品名称：剑冠丹  
资源分类：消耗品  
本质：魔幻本质  
价格：D+500  
数量：1颗  
生效轮次：永久

描述：  
“千里山河轻孺子，两朝剑冠恨谯周。”

效果：  
在服用后永久获得此效果，当你穿着轻甲时可以生效，若你进行招架，则你同时还能获得等同于人物最高内在强化支线等级\*2点格挡防御（D1C2B3A4S5）。当你服用剑冠丹达到四次时，将剑冠丹的最终效果变为当你进行招架时，视为再具有基于格挡的坚韧和免疫接触攻击，且获得等同于招架抵消数的格挡防御直至失去招架状态为止，代替前三次服用的效果。

造化已尽：服用四次

修仙世界效果：无特殊效果。

* * *

     Copyright ? 2025 [TMDtrpg制作组](http://www.goddessfantasy.net/bbs/index.php?board=2008.0). All Rights Reserved.